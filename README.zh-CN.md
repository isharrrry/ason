...
所有指南（服务商接入、operator 编写、客户端配置、执行模式、架构与贡献指南）都集中在 **[docs/index.zh-CN.md](docs/index.zh-CN.md)**，每篇均提供英文、中文与西班牙语版本。关于**桥**（把应用的 operator 通过 gRPC、MCP 或 HTTP/OpenAPI 发布出去，让独立的 Agent —— 或任何不含模型的其它程序 —— 驱动它），见 **[应用 / Agent 分离](docs/app-agent-separation.zh-CN.md)**，两侧各有一个 WPF 示例。它由四个包承载：`Ason.Bridge`（与传输无关的核心）、`Ason.Bridge.Grpc`、`Ason.Bridge.Mcp` 与 `Ason.Bridge.OpenApi` —— `Ason` 本身不引用其中任何一个。
