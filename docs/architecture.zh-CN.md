...
情形 5 —— 不含模型的普通程序（测试夹具、CI 步骤、自动化脚本、另一个服务）

[1] 调用方程序   gRPC 客户端、HTTP 客户端或 McpAsonBridgeClient —— 没有模型、没有提示词，
      |           脚本也不是模型写的：调用序列由程序自己组织
      |
      |  仍是边界 D：清单（用于发现）、invokeFunction、可选的一段脚本、日志
      |
      +--> [2] 应用主机   Agent 用的同一批端点

情形 5 就是"情形 1 换了个调用方"：桥里没有任何一处是 Agent 专属的。程序可以读清单来发现能调什么、调用单个 operator 方法，
...
情形 7 —— 上述任意调用方，一边看着日志（`logStream` 能力）

[调用方]  -- StreamExecution（gRPC）/ POST /script/stream（SSE）/ ason_stream_script（MCP）-->  [2] 应用主机
                                                                                                    |
    log* 事件，最后恰好一个 result | error  <--- 日志由 [2] 里的执行器抛出 ----------------------------+
        |
        +-- 经过中继（[R]）时，同一条流沿边界 E 继续传下去：中继没有执行器，
            因此它是**收集**应用自身的流，而不是自己产生日志
...
| 边界 | 协议 | 方向 | 由谁掌控 |
|---|---|---|---|
| D：调用方 ↔ 应用（Agent、测试夹具、CI 步骤、另一个服务） | gRPC、MCP 或 HTTP/OpenAPI，承载清单、`exec` 请求与函数调用 | 双向（脚本与参数下行、结果上行） | 应用（它托管这些端点）；调用方只是客户端 |
...
因此由中继持有管道，并需要一条通往应用的通道。调用方在边界 D 上为每种传输需要准备什么，见
[调用方接入](app-agent-separation.zh-CN.md#调用方接入需要知道什么需要配置什么)。
