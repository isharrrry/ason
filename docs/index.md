...
| **Application / agent separation** | [English](app-agent-separation.md) · [中文](app-agent-separation.zh-CN.md) · [Español](app-agent-separation.es.md) | Splitting an application that only holds `[Ason*]` operators and a bridge from the agent that drives it: the manifest, the script and single-function interfaces, the gRPC and MCP adapters, execution location and security |
