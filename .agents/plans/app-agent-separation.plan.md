...
- [ ] Task 11：`ConsoleBridgeAppSample` / `ConsoleBridgeCallerSample` 重命名到位、`git ls-files` 无残留引用、sln/CI/三语文档/E2E 助手同步、形态矩阵含角色图例（或按备选记录"仅图例"决定）
...
| 下一步（按 §5 顺序） | ⏳ 待开工 | T11（`ConsoleBridgeAppSample`/`ConsoleBridgeCallerSample` 重命名 + 角色图例），随后 T12（调用方接入矩阵）、T13（WPF `--execution` / remote-runner / docker 上报）、T14（MCP 示例 + Python+OpenAI 自动调用）、T6（覆盖率）、T7（release notes/CHANGELOG）、T8（CI 门禁）、T9（收尾） |
